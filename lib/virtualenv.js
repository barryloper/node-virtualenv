...
/*
TODO: 
do we need glob? It doesn't seem to handle windows path names
can this be used to run python scripts from multiple virtualenvs within the same Node processes?
I don't think the original code could do that, and the virtualenv setup seems to be set in the package.json
If we moved the setup from the package.json into the contstructor, virtualenvs could be set up on the fly and python
could be created/updated on a per-child-process.spawn basis
*/
...
function VirtualEnv(packageRoot) {
  // packageRoot: The folder that contains your package.json and .node-virtualenv

  var package = require(path.join(packageRoot, "./package.json"));
  package.virtualenv = package.virtualenv || {};
  this._version = package.virtualenv.version || "*";
  this._createFlags = package.virtualenv.flags || [];
  this._requirements = path.join(packageRoot, ".node-virtualenv-requirements.txt");
  this._requirementsList = package.virtualenv.requirements || ["requirements.txt"];
  if (!Array.isArray(this._requirementsList)) {
    this._requirementsList = [this._requirementsList];
  }

  // Add the specified requirements into one file via pip's -r syntax
  var _requirementsFileData = '';
  this._requirementsList.forEach(function(element, index, array){
    if (!fs.existsSync(element)) {
      throw new Error("missing " + element + " in " + moduleDir);
    } else {
      _requirementsFileData += "-r " + element + os.EOL;
    }
  });
  fs.writeFileSync(this._requirements, _requirementsFileData);

  // Define our paths.
  this._virtualenvHome = path.join(packageRoot, ".node-virtualenv");
  this._virtualenvBin = path.join(this._virtualenvHome, "bin");
  this._virtualenvPython = path.join(this._virtualenvBin, "python");
  ...
  // Setup the environment variables necessary to spawn processes within the virtualenv
  process.env.VIRTUAL_ENV = this._virtualenvHome;
  
  if (process.env.hasOwnProperty('PYTHONHOME')) {
    delete process.env.PYTHONHOME;
  };

  // Split process.env.PATH by path.delimeter (: or ;)
  // check if an os-normalized version of _virtualenvBin is in it.
  // if not, add it to the PATH string
  var _paths = process.env.PATH.split(path.delimiter);
  if (_paths.indexOf(this._virtualenvBin) == -1) {
    process.env.PATH = [this._virtualenvBin, process.env.PATH].join(path.delimiter);
  };
...
VirtualEnv.prototype.spawn = function(command, args, options) {
  return childProcess.spawn(command, args, options);
}

VirtualEnv.prototype.spawnSync = function(command, args, options) {
  return childProcess.spawnSync(command, args, options);
}

VirtualEnv.prototype.spawnPython = function(args, options) {
  return childProcess.spawn(this._virtualenvPython, args, options);
}

VirtualEnv.prototype.spawnPythonSync = function(args, options) {
  return childProcess.spawnSync(this._virtualenvPython, args, options);
}

VirtualEnv.prototype.spawn32bitOSXPython = function(args, options) {
  return childProcess.spawn("arch", ["-i386"].concat([this._virtualenvPython]).concat(args), options);
}

VirtualEnv.prototype._reportProgress = function(action, target) {
  this._stdout.write(action + " " + target + "\n");
}

VirtualEnv.prototype._find = function(callback) {
