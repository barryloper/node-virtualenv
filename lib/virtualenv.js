...
function VirtualEnv(packagePath) {
  ...
  // Concatinate the specified requirements into one file
  this._requirementsFileData = '';
  var self=this;
  this._requirementsList.forEach(function(element, index, array){
    var _requirement = path.join(path.dirname(packagePath), element);
    if (!fs.existsSync(_requirement)) {
      throw new Error("missing " + element + " in " + path.dirname(packagePath));
    } else {
      self._requirementsFileData += fs.readFileSync(_requirement) + "\n";
    }
  });
  fs.writeFileSync(".node-virtualenv-requirements.txt", this._requirementsFileData);
  this._requirements = path.join(path.dirname(packagePath), ".node-virtualenv-requirements.txt");
  ...
  //FIXME: this doesn't work because of the / in the virtualenvHome
  var fslash = new RegExp('/', 'g');
  var dot = new RegExp('\\.', 'g');
  var escaped_venv_path = this._virtualenvBin.replace(fslash, '\\/');
  escaped_venv_path = escaped_venv_path.replace(dot, '\\.');
  
  if (process.env.PATH.search('^' + escaped_venv_path) === -1){
    process.env.PATH = [this._virtualenvBin, process.env.PATH].join(':');
  };
