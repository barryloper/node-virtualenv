...
function VirtualEnv(packagePath) {
  ...
  for (var i=0; i<this._requirementsList.length(); i++) { 
      var _requirement = path.join(path.dirname(packagePath), this._requirementsList[i]);
      if (!fs.existsSync(_requirement)) {
        throw new Error("missing " + this._requirementsList[i] + " in " + path.dirname(packagePath));
      } else {
        this._requirementsFileData += fs.readFileSync(_requirement);
      }
    });
  fs.writeFileSync(".node-virtualenv-requirements.txt", this._requirementsFileData);
  this._requirements = ".node-virtualenv-requirements.txt";
