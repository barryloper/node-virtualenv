...
function VirtualEnv() {
  // Details come directly from package.json in process.cwd()
  var moduleDir = process.cwd();
  var package = require(path.join(moduleDir, "./package.json"));
  package.virtualenv = package.virtualenv || {};
  this._version = package.virtualenv.version || "*";
  this._createFlags = package.virtualenv.flags || [];
  this._requirements = path.join(moduleDir, ".node-virtualenv-requirements.txt");
  ...
  // Define our paths.
  this._virtualenvHome = path.join(moduleDir, ".node-virtualenv");
